...
const toneStyles = {
  neutral: {
    section: "border-[var(--border-ink)]",
    icon: "text-[var(--text-muted)]",
    dot: "bg-[var(--text-muted)]",
  },
  info: {
    section: "border-[var(--accent-blue)]/35",
    icon: "text-[var(--accent-blue)]",
    dot: "bg-[var(--accent-blue)]",
  },
  danger: {
    section: "border-[var(--accent-coral)]/35",
    icon: "text-[var(--accent-coral)]",
    dot: "bg-[var(--accent-coral)]",
  },
  success: {
    section: "border-[var(--accent-mint)]/35",
...
export const TaskListSection: React.FC<TaskListSectionProps> = ({
  ...
  return (
    <section className={`overflow-hidden rounded-2xl border bg-[var(--bg-surface)] ${colors.section}`}>
      <div className="flex min-h-12 items-center gap-2 border-b border-[var(--border-ink-muted)] px-3">
        <button
          type="button"
          onClick={() => setCollapsed(!isCollapsed)}
          aria-expanded={!isCollapsed}
          className="flex min-w-0 flex-1 items-center gap-2.5 py-2 text-left transition-colors hover:text-[var(--text-main)] active:scale-[0.99]"
        >
