...
export const PlannerQuickCreatePopover: React.FC<PlannerQuickCreatePopoverProps> = ({
  ...
  return (
    <div
      ...
      className="z-[99999] rounded-2xl border-[1.5px] border-[#262626] bg-white dark:bg-[#1E1E20] shadow-[3px_3px_0px_#262626] p-3.5 select-none animate-in fade-in zoom-in-95 duration-100"
      onClick={(e) => e.stopPropagation()}
    >
      {/* 1. Header: Segmented Switch & Actions */}
      <div className="flex items-center justify-between gap-2 pb-2.5 border-b border-[#E5E5EA] dark:border-[#2C2C2E]">
        {/* Switch Công việc / Sự kiện */}
        <div className="flex items-center p-0.5 rounded-xl bg-[#F2F2F7] dark:bg-[#2C2C2E] border border-[#E5E5EA] dark:border-[#3A3A3C]">
          <button
            type="button"
            onClick={() => onItemTypeChange("task")}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold transition-all cursor-pointer ${
              itemType === "task"
                ? "bg-[var(--accent-sky)] text-[#1C1917] shadow-xs"
        ...
        <div className="flex items-center gap-1">
          {onOpenFullDetail && (
            <button
              ...
              className="p-1.5 text-[#8E8E93] hover:text-[#1C1C1E] dark:hover:text-white hover:bg-black/[0.05] dark:hover:bg-white/[0.08] rounded-lg transition-all cursor-pointer"
      ...
      <div className="py-3 space-y-2.5">
        <input
          ...
          className="w-full bg-[#F2F2F7] dark:bg-[#2C2C2E] border-[1.5px] border-[#262626] dark:border-[#3A3A3C] rounded-xl px-3 py-2 text-sm font-bold text-[#1C1C1E] dark:text-white placeholder:text-[#8E8E93] focus:outline-none focus:ring-1 focus:ring-[var(--accent-blue)] transition-all"
        />

        {/* Thông tin mốc thời gian */}
        <div className="flex items-center gap-2 text-xs font-semibold text-[#8E8E93] dark:text-[#A1A1AA] px-1">
          <Clock size={13} strokeWidth={2.2} className="text-[var(--accent-blue)] shrink-0" />
          <span className="font-mono text-[#1C1917] dark:text-white">{timeRangeDisplay}</span>
          <span>·</span>
      ...
      <div className="flex items-center justify-between gap-2 pt-2 border-t border-[#E5E5EA] dark:border-[#2C2C2E]">
        {onOpenFullDetail ? (
          <button
            type="button"
            onClick={onOpenFullDetail}
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="px-3 py-1.5 rounded-xl border border-[#E5E5EA] dark:border-[#2C2C2E] bg-[var(--bg-surface)] hover:bg-[var(--bg-surface-muted)] text-xs font-bold text-[#78716C] dark:text-[#A1A1AA] active:scale-95 transition-all cursor-pointer"
          >
            Hủy
          </button>
