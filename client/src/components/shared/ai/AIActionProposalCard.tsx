...
export const AIActionProposalCard: React.FC<AIActionProposalCardProps> = ({ proposal }) => {
  ...
  return (
    <div className="mt-3 rounded-2xl bg-black/[0.03] dark:bg-white/[0.05] p-3.5 shadow-xs">
      <div className="flex items-start gap-2.5">
        <div className="mt-0.5 shrink-0 text-[#007AFF] dark:text-[#0A84FF]">
          <ClipboardList size={16} strokeWidth={2.2} />
        </div>
        <div className="min-w-0 flex-1">
          <p className="text-xs font-bold text-[#1C1C1E] dark:text-[#F2F2F7]">Đề xuất thay đổi</p>
          <div className="mt-2 space-y-1.5">
            {proposal.actions.map((action, index) => (
              <div
                key={`${proposal.id}-${index}`}
                className="flex items-center gap-1.5 text-xs text-[#57534E] dark:text-[#D4D4D8]"
              >
                <span className="shrink-0 text-[#8E8E93] dark:text-[#A1A1A6]">{actionIcon(action)}</span>
                <span>{actionLabel(action)}</span>
                {action.type === "complete_task" || action.type === "delete_task" ? (
                  <span className="truncate text-[#8E8E93] dark:text-[#A1A1A6]">
          ...
          <button
            ...
            className={`mt-3 inline-flex min-h-8 items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold rounded-2xl shadow-2xs transition-all cursor-pointer disabled:cursor-not-allowed disabled:opacity-60 ${
              isApplied
                ? "bg-emerald-600 text-white"
                : "bg-[#007AFF] hover:bg-[#0071E3] dark:bg-[#0A84FF] dark:hover:bg-[#0071E3] text-white"
            }`}
          >
