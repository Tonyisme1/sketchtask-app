...
export class SyncService {
   ...
   */
  static async syncData(userId: string, payload: Partial<SyncPayload>) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (payload.weeklyReflection !== undefined && payload.weeklyReflection !== null) {
        ...
        if (typeof payload.weeklyReflection === "string") {
          ...
          reflectionText = (payload.weeklyReflection as any).text || "";
          if ((payload.weeklyReflection as any).updatedAt) {
            itemUpdatedAt = new Date((payload.weeklyReflection as any).updatedAt);
          } else if (defaultPayloadTime) {
            itemUpdatedAt = defaultPayloadTime;
          }
        }
